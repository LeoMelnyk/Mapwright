...
function openEventSource(url: string) {
  ...
  eventSource.onmessage = (e: MessageEvent) => {
    ...
    try {
      ...
      console.warn('[downloader] malformed SSE message', String(e.data).slice(0, 120));
      return;
    }
    ...
    switch (msg.type) {
      ...
      case 'file_start': {
        const fb = (fileBars as Record<string, { bar: HTMLElement; size: HTMLElement } | undefined>)[msg.file];
        if (fb) {
          fb.bar.style.width = '0%';
          fb.size.textContent = 'Downloading…';
        }
        break;
      }
      ...
      case 'file_progress': {
        const fb = (fileBars as Record<string, { bar: HTMLElement; size: HTMLElement } | undefined>)[msg.file];
        if (fb) {
          const pct = msg.totalBytes > 0 ? Math.round((msg.bytesReceived / msg.totalBytes) * 100) : 0;
          fb.bar.style.width = pct + '%';
          fb.size.textContent =
            msg.totalBytes > 0
      ...
      case 'file_done': {
        ...
        if (fb) {
          if (msg.status === 'failed') {
            fb.bar.style.width = '0%';
            fb.size.textContent = 'Failed';
          } else if (msg.status === 'exists') {
            fb.bar.style.width = '100%';
            fb.size.textContent = `${formatBytes(msg.totalBytes)} (exists)`;
          } else if (msg.status === 'unavailable') {
            fb.size.textContent = 'N/A';
          }
        }
        break;
      }
      ...
      case 'complete':
        ...
        downloadingSection.style.display = 'none';
        completeText.textContent = `Download complete! ${msg.downloaded} downloaded, ${msg.skipped} already existed${msg.failed > 0 ? `, ${msg.failed} failed` : ''}.`;
        if (msg.failures?.length) {
          console.error('[Textures] Failed textures:', msg.failures);
          failedList.innerHTML =
            '<div class="failed-label">Failed textures:</div>' +
            msg.failures
              .map((f: { name: string; reason: string }) => `<div class="failed-item">${f.name} — ${f.reason}</div>`)
              .join('');
        }
...
function startDownload(mode: string) {
  ...
  const url = mode === 'required' ? '/api/textures/download?mode=required' : '/api/textures/download';
