...
interface DonjonData {
  ...
  rooms: {
    ...
    west: number;
    shape?: string;
    doors?: Record<string, { row: number; col: number }[]>;
  }[];
...
export function convertDonjonDungeon(data: DonjonData): { metadata: Metadata; cells: CellGrid[][] } {
  const bitsRaw = data.cell_bit;
  const bits = {
    room: bitsRaw.room!,
    corridor: bitsRaw.corridor!,
    perimeter: bitsRaw.perimeter!,
    aperture: bitsRaw.aperture!,
    arch: bitsRaw.arch!,
    door: bitsRaw.door!,
    locked: bitsRaw.locked!,
    trapped: bitsRaw.trapped!,
    secret: bitsRaw.secret!,
    portcullis: bitsRaw.portcullis!,
    stair_down: bitsRaw.stair_down!,
    stair_up: bitsRaw.stair_up!,
    room_id: bitsRaw.room_id!,
    label: bitsRaw.label!,
  };
  ...
  let roomOffR = 0,
    roomOffC = 0;
  // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition
  const firstRoom = (((data as Record<string, unknown>).rooms as Record<string, unknown>[]) || []).find(
    (r: unknown) => r,
  );
  if (firstRoom) {
    const targetId = parseInt(firstRoom.id as string);
    outer: for (let r = 0; r < origRows; r++) {
      for (let c = 0; c < origCols; c++) {
  ...
  for (const room of data.rooms || []) {
      ...
      for (const door of doorList) {
        ...
        if (axis === 'ew') {
          ...
          for (let dc = 0; dc < RES; dc++) {
            ...
            const lo = Math.min(rFrom, rTo),
              hi = Math.max(rFrom, rTo);
    ...
    if (room.shape === 'circle') {
      ...
      for (const cn of ['nw', 'ne', 'sw', 'se']) {
        ...
        for (let r = rStart; r <= rEnd; r++) {
          for (let c = cStart; c <= cEnd; c++) {
            ...
            const origV = src[origR]?.[origC];
            if (!origV || !(origV & bits.room)) continue;
            ...
            if (arcData) {
              ...
              for (const dir of ['north', 'south', 'east', 'west']) {
                ...
                if (
                  nb?.[OPP[dir as keyof typeof OPP]] &&
                  nb[OPP[dir as keyof typeof OPP]] !== 'd' &&
                  nb[OPP[dir as keyof typeof OPP]] !== 's'
                )
                  delete nb[OPP[dir as keyof typeof OPP]];
              }
              tc.trimCorner = cn;
              tc.trimClip = arcData.trimClip;
              tc.trimWall = arcData.trimWall;
              tc.trimCrossing = arcData.trimCrossing;
            } else {
      ...
      const sz =
        Math.max(
          1,
          Math.floor(minDim_orig / ((((room as Record<string, unknown>).polygon as number) || 6) <= 6 ? 3 : 4)),
        ) * RES;
      if (sz >= RES) {
        const cornerDefs = [
    ...
    for (const [key, axis] of punchThrough) {
      ...
      for (const [dir, dr, dc] of sides) {
        ...
        if (!nb) {
          ...
          if (nb.trimCorner.includes(FACING[dir])) {
            cell[dir] ??= 'w';
          }
        }
      }
    ...
    for (let c = 0; c < cols; c++) {
      ...
      if (origVal & bits.corridor) {
        ...
        for (const [dr, dc] of [
          ...
          if (nv & bits.room && (nv & bits.room_id) >> 6 !== myRoomId) {
            restore = true;
            break;
          }
        }
  ...
  for (const key of restoredCells) {
    const [r, c] = (key as string).split(',').map(Number) as [number, number];
    const cell = cells[r]?.[c];
    if (!cell) continue;
