...
export const TOOL_DEFINITIONS = [
  {
    name: 'getMapInfo',
    description:
      'Get current map metadata: name, dimensions, theme, room count, etc. Call this first to understand the current state of the map.',
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        theme: {
          type: 'string',
          description: 'Theme name, e.g. "stone-dungeon", "crypt", "earth-cave", "ice-cave", "volcanic", "arcane"',
        },
      },
  ...
  },
  {
    name: 'createRoom',
    description:
      'Create a walled rectangular room. Coordinates are row, col (0-indexed, row 0 = top). Use mode "merge" when adding a room adjacent to an existing one to avoid double walls.',
    input_schema: {
      type: 'object',
      properties: {
        ...
        mode: {
          type: 'string',
          enum: ['room', 'merge'],
          description: '"room" walls all edges (default). "merge" only walls edges facing void.',
        },
      },
  ...
  },
  {
    name: 'setLabel',
    description:
      'Set a room reference label at a cell. MUST be {Letter}{Number} format (e.g. "A1", "B3", "C12") — NO descriptive names like "Guard Room". The next available label is shown in the map context as "Next room label". Place at the cell closest to the room center.',
    input_schema: {
      type: 'object',
      properties: {
        row: { type: 'integer' },
        ...
        text: {
          type: 'string',
          description:
            'Room label in {Letter}{Number} format, e.g. "A1". Use the "Next room label" value from the map context.',
        },
      },
  ...
  },
  {
    name: 'setDoor',
    description:
      'Place a door on a wall between two rooms. The door is placed on the shared wall. Use type "d" for a normal door, "s" for a secret door.',
    input_schema: {
      type: 'object',
      properties: {
        ...
        direction: {
          ...
          description: 'Which wall of the cell to place the door on',
        },
    ...
    input_schema: {
      ...
      properties: {
        theme: {
          type: 'string',
          description:
            'Theme name: "stone-dungeon", "crypt", "earth-cave", "ice-cave", "water-temple", "underdark", "volcanic", "swamp", "desert", "dirt", "grasslands", "snow-tundra", "arcane", "alien", "blue-parchment", "sepia-parchment"',
        },
      },
      required: ['theme'],
    },
  ...
  },
  {
    name: 'getRoomBounds',
    description:
      'Get the bounding box and center of a labeled room. Use to find good positions for doors, props, and labels.',
    input_schema: {
      type: 'object',
  ...
  },
  {
    ...
    description:
      'Find all wall positions on the shared boundary between two labeled rooms. Use to pick the best door location.',
    input_schema: {
      type: 'object',
      properties: {
        label1: { type: 'string' },
  ...
  },
  {
    name: 'addStairs',
    description:
      'Place stairs using 3 corner points (grid-corner coordinates, not cell centers). P1→P2 = base edge (hatch lines start here); P3 = depth target. Examples: 3-cell rectangle stair facing south — p1r=5,p1c=2, p2r=5,p2c=5, p3r=4,p3c=5. Single-cell stair — p1r=5,p1c=5, p2r=5,p2c=6, p3r=4,p3c=6.',
    input_schema: {
      type: 'object',
      properties: {
        p1r: { type: 'integer' },
  ...
  },
  {
    name: 'listProps',
    description:
      'List all available prop types organized by category. Call this before using placeProp to see valid prop names.',
    input_schema: { type: 'object', properties: {}, required: [] },
  },
  {
    name: 'placeLight',
    description:
      'Place a light source at world-feet coordinates. IMPORTANT: x = col × gridSize, y = row × gridSize — NOT raw row/col. For a 5ft grid, cell (row=3, col=5) → x=25, y=15. Use preset names like "torch", "candle", "brazier" — call listLightPresets for all names.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  },
  {
    name: 'listTextures',
    description:
      'List all available texture IDs with display names and categories. Call this before using setTexture or setTextureRect to find valid texture IDs.',
    input_schema: { type: 'object', properties: {}, required: [] },
  },
  {
    name: 'setTexture',
  ...
  },
  {
    name: 'setTextureRect',
    description:
      'Apply a texture to every cell in a rectangle. One undo step. Call listTextures first to find valid texture IDs.',
    input_schema: {
      type: 'object',
      properties: {
        r1: { type: 'integer' },
  ...
  },
  {
    ...
    description:
      'Flood-fill a texture starting from a cell, spreading to all connected non-void cells that share the same current texture state (including cells with no texture). Click any cell in a room to paint the entire connected floor area in one call. Call listTextures first for valid IDs.',
    input_schema: {
      type: 'object',
      properties: {
        row: { type: 'integer' },
  ...
  },
  {
    name: 'paintCell',
    description:
      'Paint a single cell as a floor (makes it non-void). Use this to carve irregular room shapes or corridors cell by cell.',
    input_schema: {
      type: 'object',
      properties: {
        row: { type: 'integer' },
  ...
  },
  {
    name: 'addBridge',
    description:
      'Place a bridge using 3 corner points (same system as addStairs). P1→P2 = bridge span; P3 = depth (typically 1 row beyond P2). Example: 3-cell wood bridge spanning east across row 5 — type="wood", p1r=5,p1c=3, p2r=5,p2c=6, p3r=4,p3c=6. Good for crossing water, pits, or chasms.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  },
  {
    name: 'linkStairs',
    description:
      'Link two stair objects together (they share a letter label like A, B, C to show they connect). Pass the row/col of each stair.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description:
      'Get all contents of a labeled room: props, fills (water/pit/lava), textures, and doors. Use this before modifying a room to avoid duplicating existing elements.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description:
      'Find a free rectangular area of the given size on the map. Use this before createRoom to avoid placing rooms in occupied space. Returns { r1, c1, r2, c2 } of a valid placement.',
    input_schema: {
      type: 'object',
      properties: {
        ...
        adjacentTo: {
          type: 'string',
          description: 'Optional room label — if given, prefers placements adjacent to that room',
        },
      },
      required: ['rows', 'cols'],
    },
  },
  {
    name: 'createCorridor',
    description:
      'Create a walled 2-cell-wide corridor connecting two labeled rooms. Rooms must be axis-aligned with perpendicular overlap. Automatically assigns the next room label and places doors at both ends. Use this instead of manually computing corridor coordinates.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  // ── Lighting ─────────────────────────────────────────────────────────────
  {
    ...
    description:
      'List all available light presets (torch, candle, brazier, lantern, etc.) with their properties. Call this before placeLight to find valid preset names.',
    input_schema: { type: 'object', properties: {}, required: [] },
  },
  {
    name: 'getLights',
  ...
  },
  {
    ...
    description:
      'Set the ambient light level (0.0 = pitch black, 1.0 = fully lit, 0.1–0.3 typical for a dark dungeon). Only affects maps with lighting enabled.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  // ── Label style ───────────────────────────────────────────────────────────
  {
    ...
    input_schema: {
      ...
      properties: {
        style: {
          type: 'string',
          enum: ['circled', 'plain', 'bold'],
          description: '"circled" = number in a circle (default), "plain" = plain text, "bold" = bold text',
        },
      },
      required: ['style'],
    },
  ...
  // ── Spatial utilities ─────────────────────────────────────────────────────
  {
    ...
    description:
      'Find the row/col of the cell containing a given room label text (e.g. "A1"). Returns { row, col } or null.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  },
  {
    name: 'shiftCells',
    description:
      'Shift all map content by dr rows and dc cols. Expands the grid if needed. Use this to reposition the entire dungeon on the canvas.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  // ── AI convenience tools ──────────────────────────────────────────────────
  {
    name: 'listRooms',
    description:
      'List all labeled rooms on the map with their bounding boxes and centers. Returns [{label, r1, c1, r2, c2, center:{row,col}}]. Use this to find room positions before placing props, lights, or routing corridors.',
    input_schema: { type: 'object', properties: {}, required: [] },
  },
  {
    name: 'placeLightInRoom',
    description:
      'Place a light at the center of a labeled room. Handles world-feet coordinate conversion automatically — no math needed. Use a preset name like "torch", "candle", "brazier" — call listLightPresets for valid names.',
    input_schema: {
      type: 'object',
      properties: {
        label: { type: 'string', description: 'Room label, e.g. "A1"' },
...
const CORE_TOOL_NAMES = new Set(['getMapInfo']);
...
export function executeTool(name: string, input: Record<string, string | number | boolean>): Record<string, unknown> {
  ...
  const api = (window as unknown as Record<string, unknown>).editorAPI as Record<
    string,
  ...
  switch (name) {
    case 'createRoom':
      ...
      if ((input.r1 as number) > (input.r2 as number))
        return { error: `createRoom: r1 (${input.r1}) must be ≤ r2 (${input.r2})` };
      if ((input.c1 as number) > (input.c2 as number))
        return { error: `createRoom: c1 (${input.c1}) must be ≤ c2 (${input.c2})` };
      break;
    case 'setLabel':
      ...
      if (!/^[A-Z]\d+$/.test(String(input.text))) {
        return {
          error: `setLabel: "${input.text}" is not valid. Must be {UppercaseLetter}{Number} format (e.g. A1, B3). Use the "Next room label" value from the map context.`,
        };
      }
      break;
    case 'setDoor':
    ...
    case 'removeWall':
      ...
      if (err) return { error: `${name}: ${err}` };
      if (
        !VALID_DIRECTIONS.has(String(input.direction)) &&
        input.direction !== 'nw-se' &&
        input.direction !== 'ne-sw'
      ) {
        return {
          error: `${name}: direction must be "north", "south", "east", "west", "nw-se", or "ne-sw". Got "${input.direction}".`,
        };
      }
      break;
    case 'setFill':
    ...
    case 'setFillRect':
      ...
      if (!VALID_FILLS.has(String(input.fillType))) {
        return {
          error: `setFillRect: fillType must be one of: ${[...VALID_FILLS].join(', ')}. Got "${input.fillType}".`,
        };
      }
      break;
    case 'setTextureRect':
  ...
  try {
    switch (name) {
      ...
      case 'getBridges':
        return fn();
      case 'newMap':
        return fn(input.name, input.rows, input.cols, input.gridSize, input.theme);
      case 'createRoom': {
        ...
        if (roomResult.success && input.label && /^[A-Z]\d+$/.test(String(input.label))) {
          try {
            ...
            /* room may not have center cell yet — model should call setLabel separately */
          }
        }
        return roomResult;
      }
      ...
      case 'setDoor':
        return fn(input.row, input.col, input.direction, input.type);
      case 'setWall':
        return fn(input.row, input.col, input.direction);
      case 'removeWall':
        return fn(input.row, input.col, input.direction);
      case 'setFill':
        return fn(input.row, input.col, input.fillType, input.depth);
      case 'setFillRect':
        return fn(input.r1, input.c1, input.r2, input.c2, input.fillType, input.depth);
      case 'setTheme':
        return fn(input.theme);
      case 'setName':
        return fn(input.name);
      case 'setFeature':
        return fn(input.feature, input.enabled);
      case 'getCellInfo':
        return fn(input.row, input.col);
      case 'getRoomBounds':
        return fn(input.label);
      case 'findWallBetween':
        return fn(input.label1, input.label2);
      case 'addStairs':
        return fn(input.p1r, input.p1c, input.p2r, input.p2c, input.p3r, input.p3c);
      case 'placeProp':
        return fn(input.row, input.col, input.propType, input.facing);
      case 'placeLight':
        return fn(input.x, input.y, input.config);
      case 'mergeRooms':
        return fn(input.label1, input.label2);
      case 'createTrim':
        return fn(input.r1, input.c1, input.r2, input.c2, input.options);
      case 'setTexture':
        return fn(input.row, input.col, input.textureId, input.opacity);
      case 'setTextureRect':
        return fn(input.r1, input.c1, input.r2, input.c2, input.textureId, input.opacity);
      case 'floodFillTexture':
        return fn(input.row, input.col, input.textureId, input.opacity);
      case 'removeTexture':
        return fn(input.row, input.col);
      case 'removeTextureRect':
        return fn(input.r1, input.c1, input.r2, input.c2);
      case 'paintCell':
        return fn(input.row, input.col);
      case 'paintRect':
        return fn(input.r1, input.c1, input.r2, input.c2);
      ...
      case 'rotateProp':
        return fn(input.row, input.col);
      case 'setHazard':
        return fn(input.row, input.col, input.enabled);
      case 'setHazardRect':
        return fn(input.r1, input.c1, input.r2, input.c2, input.enabled);
      case 'addBridge':
        return fn(input.type, input.p1r, input.p1c, input.p2r, input.p2c, input.p3r, input.p3c);
      case 'removeBridge':
        return fn(input.row, input.col);
      case 'linkStairs':
        return fn(input.r1, input.c1, input.r2, input.c2);
      ...
      case 'suggestPlacement':
        return fn(input.rows, input.cols, input.adjacentTo);
      case 'createCorridor':
        return fn(input.label1, input.label2, input.width);
      // Props
      case 'removeProp':
        return fn(input.row, input.col);
      case 'removePropsInRect':
        return fn(input.r1, input.c1, input.r2, input.c2);
      // Stairs
      case 'removeStairs':
        return fn(input.row, input.col);
      // Lighting
      case 'listLightPresets':
        return fn();
      case 'getLights':
        return fn();
      case 'removeLight':
        return fn(input.id);
      case 'setAmbientLight':
        return fn(input.level);
      case 'setLightingEnabled':
        return fn(input.enabled);
      // Label style
      case 'setLabelStyle':
      ...
      case 'getLevels':
        return fn();
      case 'addLevel':
        return fn(input.name, input.numRows);
      case 'renameLevel':
        return fn(input.levelIndex, input.name);
      case 'resizeLevel':
        return fn(input.levelIndex, input.numRows);
      // Spatial
      case 'findCellByLabel':
        return fn(input.label);
      case 'shiftCells':
        return fn(input.dr, input.dc);
      // AI convenience
      case 'listRooms':
        return fn();
      case 'placeLightInRoom':
        return fn(input.label, input.preset);
      default:
        return { error: `No dispatch for tool: "${name}". This tool exists but has no handler — report this bug.` };
    }
  } catch (execErr) {
