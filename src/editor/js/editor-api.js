...
const api = {
  ...
  /**
   * Create a room from an arbitrary list of cells (non-rectangular shapes).
   * mode='room': wall every exterior edge (overrides existing walls).
   * mode='merge': wall only edges facing void; open edges facing existing cells.
   * One undo step.
   */
  createPolygonRoom(cellList, mode = 'room') {
  ...
  listRoomCells(label) {
    const roomCells = this._collectRoomCells(label);
    if (!roomCells.size) return { error: `Room "${label}" not found or empty` };
    const cells = [];
   ...
   */
  getValidPropPositions(label, propType, facing = 0) {
    const catalog = state.propCatalog;
    if (!catalog?.props[propType]) {
      throw new Error(`Unknown prop type: ${propType}. Available: ${Object.keys(catalog?.props || {}).join(', ')}`);
    }
    ...
    const roomCellSet = this._collectRoomCells(label);
    if (!roomCellSet.size) return { error: `Room "${label}" not found` };
  ...
  planBrief(brief) {
    ...
    for (const conn of connections) {
      ...
      (adj[conn.to] = adj[conn.to] || []).push({ to: conn.from, dir: ODIR[conn.direction], corrW, type });
    }
    ...
    while (queue.length) {
      ...
      for (const [dir, edges] of Object.entries(byDir)) {
        ...
        if (isNS) {
          ...
          const totalW = edges.reduce((s, e) => s + roomMap[e.to].width, 0) + (edges.length - 1);
          ...
          const totalH = edges.reduce((s, e) => s + roomMap[e.to].height, 0) + (edges.length - 1);
