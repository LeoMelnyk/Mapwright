 ...
 */
export function renderFloors(ctx, cells, roomCells, gridSize, theme, transform, textureOptions, bgImageEl = null, bgImgConfig = null, visibleBounds = null, resolution = 1) {
 ...
 * @returns {void}
 */
export function renderTextureBlending(ctx, cells, roomCells, gridSize, transform, textureOptions, cacheSize = null) {
 ...
 * @returns {void}
 */
export function renderFillPatternsAndGrid(ctx, cells, roomCells, gridSize, theme, transform, showGrid, skipGrid = false, metadata = null, cacheSize = null) {
 ...
 * @returns {void}
 */
export function renderHazardOverlay(ctx, cells, gridSize, transform) {
  const numRows = cells.length;
  const numCols = cells[0]?.length || 0;
 ...
 * @returns {void}
 */
export function renderWallsAndBorders(ctx, cells, roomCells, gridSize, theme, transform, showInvisible = false, visibleBounds = null, _res = 1) {
  const numRows = cells.length;
  const numCols = cells[0]?.length || 0;
  ...
  for (let row = startRow; row <= endRow; row++) {
    for (let col = startCol; col <= endCol; col++) {
      ...
      if (theme.wallShadow) {
        ...
        const sx = 2 * s, sy = 2 * s;
        ctx.beginPath();
        const p0s = toCanvas(cx + wall[0][0] * gridSize + sx / transform.scale, cy + wall[0][1] * gridSize + sy / transform.scale, transform);
        ctx.moveTo(p0s.x, p0s.y);
        for (let i = 1; i < wall.length; i++) {
          const p = toCanvas(cx + wall[i][0] * gridSize + sx / transform.scale, cy + wall[i][1] * gridSize + sy / transform.scale, transform);
          ctx.lineTo(p.x, p.y);
        }
        ctx.stroke();
        ctx.restore();
      ...
      ctx.lineCap = 'round';
      ctx.beginPath();
      const p0 = toCanvas(cx + wall[0][0] * gridSize, cy + wall[0][1] * gridSize, transform);
      ctx.moveTo(p0.x, p0.y);
      for (let i = 1; i < wall.length; i++) {
        const p = toCanvas(cx + wall[i][0] * gridSize, cy + wall[i][1] * gridSize, transform);
        ctx.lineTo(p.x, p.y);
      }
      ctx.stroke();
      ctx.restore();
 ...
 * @returns {void}
 */
export function renderLabelsStairsProps(ctx, cells, gridSize, theme, transform, labelStyle, propCatalog, textureOptions, metadata, skipLabels = false, visibleBounds = null, cacheSize = null) {
  const numRows = cells.length;
  const numCols = cells[0]?.length || 0;
