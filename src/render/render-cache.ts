...
function _cellHasFluid(cell: any): boolean {
...
export function smartInvalidate(changes: BeforeState[], cells: CellGrid, { forceGeometry = false, forceFluid = false }: { forceGeometry?: boolean; forceFluid?: boolean } = {}): void {
  ...
  if (needsFluid) {
    if (!forceFluid && dirtyRegion && getFluidCacheParams()) {
...
function _patchBlendFromCache(cells: CellGrid, region: any): void {
 ...
 * @returns {void}
 */
export function patchBlendForDirtyRegion(region: any, cells: CellGrid, gridSize: number, textureOptions: any): void {
  const roomCells = getCachedRoomCells(cells);
 ...
 * @returns {void}
 */
export function patchFluidForDirtyRegion(region: any, cells: CellGrid, gridSize: number, theme: Theme): void {
  const roomCells = getCachedRoomCells(cells);
 ...
 */
export function collectRoundedCorners(cells: CellGrid): Map<string, any> {
 ...
 * @returns {void}
 */
export function traceArcWedge(ctx: CanvasRenderingContext2D, rc: any, gridSize: number, transform: RenderTransform): void {
