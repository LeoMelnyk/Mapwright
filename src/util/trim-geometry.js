/**
 * trim-geometry.js — Pure geometry for computing per-cell round trim data.
 ...
 * @param {number} arcRow - Grid corner row (arcCenter.row from preview)
 * @param {number} arcCol - Grid corner col (arcCenter.col from preview)
 * @param {number} size   - Trim radius in cells
 * @param {string} corner - 'nw'|'ne'|'sw'|'se'
 * @param {boolean} inverted
 * @returns {{ cx: number, cy: number }} cx = column axis, cy = row axis
 */
export function computeCircleCenter(arcRow, arcCol, size, corner, inverted) {
...
function getArcAngleRange(corner, inverted) {
  if (!inverted) {
    switch (corner) {
...
function isAngleInRange(angle, [lo, hi]) {
...
function isRoomSide(x, y, cx, cy, R, inverted) {
 ...
 */
function findCellArcIntersections(row, col, cx, cy, R, arcRange) {
  const hits = [];
  const eps = 1e-9;

  function tryEdge(name, fixed, isHoriz, lo, hi) {
 ...
 */
function edgeParam(gx, gy, col, row) {
 ...
 * @returns {{ clip: number[][], wall: number[][] }} in cell-local [0,1] coords, or null.
 */
function buildClipAndWall(row, col, cx, cy, R, inverted, arcRange, numSamples = 24) {
  ...
  let roomCorners, roomStart, roomEnd;
  ...
  const wall = [[round3(roomEnd.x - col), round3(roomEnd.y - row)]];
  ...
  const clip = [[round3(roomStart.x - col), round3(roomStart.y - row)]];
 ...
 */
export function computeTrimCrossing(clip, wall) {
  ...
  const edgeRanges = { n: [0, 1], e: [1, 2], s: [2, 3], w: [3, 4] };
  ...
  if (sideA.length === 0 || sideB.length === 0) {
    ...
    const arcEdges = new Set([startEdge, endEdge]);
    const result = {};
 ...
 * @returns {{ trimClip, trimWall, trimPassable }|null} — null if arc doesn't intersect cell.
 */
export function computeArcCellData(row, col, cx, cy, R, corner, inverted) {
 ...
 *
 * @param {Object} preview - From _updatePreview: { hypotenuse, voided, insideArc, arcCenter: {row,col}, size }
 * @param {string} corner   - 'nw'|'ne'|'sw'|'se'
 * @param {boolean} inverted
 * @param {boolean} open
 * @returns {Map<string, Object|null|'interior'>}
 ...
 */
export function computeTrimCells(preview, corner, inverted, open) {
  ...
  for (const key of candidateKeys) {
    ...
    if (open) {
      ...
      const entry = { trimCorner: corner, trimWall: data.wall, trimClip: data.clip,
        trimCrossing: computeTrimCrossing(data.clip, data.wall) };
