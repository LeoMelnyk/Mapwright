 ...
 */
export function computeCircleCenter(
  arcRow: number,
  arcCol: number,
  size: number,
  corner: TrimCorner,
  ...
  switch (corner) {
    case 'nw':
      return { cx: arcCol + size, cy: arcRow + size };
    case 'ne':
      return { cx: arcCol - size, cy: arcRow + size };
    case 'sw':
      return { cx: arcCol + size, cy: arcRow - size };
 ...
 */
function findCellArcIntersections(
  row: number,
  col: number,
  cx: number,
  cy: number,
  ...
  for (const p of hits) {
    if (!unique.some((u) => Math.abs(u.x - p.x) < 1e-6 && Math.abs(u.y - p.y) < 1e-6)) unique.push(p);
  }
  return unique;
}
...
function buildClipAndWall(
  ...
  ints.sort((a, b) => normalizeAngle(Math.atan2(a.y - cy, a.x - cx)) - normalizeAngle(Math.atan2(b.y - cy, b.x - cx)));
  const [i1, i2] = ints as [ArcIntersection, ArcIntersection];
 ...
 */
export function computeArcCellData(
  row: number,
  col: number,
  cx: number,
  cy: number,
  ...
  corner: TrimCorner,
  inverted: boolean,
): { trimClip: number[][]; trimWall: number[][]; trimCrossing: TrimCrossing } | null {
 ...
 */
export function computeTrimCells(
  preview: TrimPreview,
  corner: TrimCorner,
  inverted: boolean,
  open: boolean,
): Map<string, TrimCellData | null | 'interior'> {
  ...
  if (inverted) {
    ...
    switch (corner) {
      case 'nw':
        r0 = arcCenter.row;
        r1 = arcCenter.row + size;
        c0 = arcCenter.col;
        c1 = arcCenter.col + size;
        break;
      case 'ne':
        r0 = arcCenter.row;
        r1 = arcCenter.row + size;
        c0 = arcCenter.col - size;
        c1 = arcCenter.col;
        break;
      case 'sw':
        r0 = arcCenter.row - size;
        r1 = arcCenter.row;
        c0 = arcCenter.col;
        c1 = arcCenter.col + size;
        break;
